...
(define tm-int
  '((read Q Right)
   ...
   (fin (return Right))
   (disp-fail (return (displayln "TM: error instruction")))))
...
(display "check `find-name`: ")
(equal? (int find-name '(y (x y z) (1 2 3))) 2)

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define tm-in (list 1 1 0 1 0 1))
(define tm-out (list 1 1 0 1))

(display "check `tm-example`: ")
(equal? (int tm-int `(,tm-example ,tm-in)) tm-out)
